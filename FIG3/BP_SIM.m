...
OP = zeros(length(PdB), length(alpha));
%
for nn = 1 : length(PdB)
for aa = 1 : length(alpha)
    OP(nn,aa) = BPfunc(PdB(nn),QdB,LL,PL,RR,NN,xB,yB,xP,yP,xE,yE,eta,alpha(aa),Num_Trial);
    if nn ==1
    h1 = semilogy(alpha,OP,'ro'); grid on;hold on;
    else
    semilogy(alpha,OP,'ro'); grid on;hold on;
    end
end
end
%
...
for aa = 1 : length(alpha)
    ...
    for bitnum   =  1 : Num_Trial
        ...
        for bb = 1 : length(LL)
            ...
            CC       = (1-alpha(aa))/Hop*log2(1 + SNRmin);
            if (CC > CC_max)
                CC_max = CC;
            end
        end
...
%OP
%
%plot(alpha,OP,'ro'); grid on;hold on;
end
