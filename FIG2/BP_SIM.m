...
OP_SIM = zeros(length(alpha), length(PdB));
%
for nn = 1 : length(alpha)
for aa = 1 : length(PdB)
    OP_SIM(nn,aa) = BPfunc(PdB(aa),QdB,LL,PL,RR,NN,xB,yB,xP,yP,xE,yE,eta,alpha(nn),Num_Trial);
    if nn ==1
    h1 = semilogy(PdB,OP_SIM,'ro'); grid on;hold on;
    else
    semilogy(PdB,OP_SIM,'ro'); grid on;hold on;
    end
end
end
%
...
%OP
%
%plot(PdB,OP,'ro'); grid on;hold on;
end
